...
We recommend piping dumps through `systemd-coredump` helper script, which stores them on the host filesystem independent of pod lifetime.

## Platform requirements

Every node must have the following in place before any crash occurs:
...
3. **`kernel.core_pattern` set** to pipe crashes through the helper.
...
:::{code-block} bash
:substitutions:
kubectl apply --server-side -f=https://raw.githubusercontent.com/{{repository}}/{{revision}}/examples/gke/coredumps/coredump-conf.configmap.yaml
:::

Adjust `MaxUse` and `KeepFree` before applying - see [Storage considerations](#storage-considerations).
