...
var _ = g.Describe("ScyllaDBManagerTask and ScyllaDBCluster integration with global ScyllaDB Manager", framework.MultiDatacenter, func() {
	...
	g.It("should synchronise a backup task for ScyllaDBCluster and support a manual restore procedure", func(ctx g.SpecContext) {
		...
		for dcIdx, dcSpec := range sc.Spec.Datacenters {
			...
			case framework.ObjectStorageTypeGCS:
				...
				o.Expect(err).NotTo(o.HaveOccurred())
				sc.Spec.Datacenters[dcIdx] = *setUpGCSCredentialsForScyllaDBClusterDatacenter(ctx, nsClient.KubeClient().CoreV1(), &dcSpec, ns.Name, gcServiceAccountKey)
			...
			case framework.ObjectStorageTypeS3:
				...
				o.Expect(err).NotTo(o.HaveOccurred())
				sc.Spec.Datacenters[dcIdx] = *setUpS3CredentialsForScyllaDBClusterDatacenter(ctx, nsClient.KubeClient().CoreV1(), &dcSpec, ns.Name, s3CredentialsFile)
			}
		}
		...
		waitSDBCRolloutCtx, cancelSDBCRollout := utils.ContextForMultiDatacenterScyllaDBClusterRollout(ctx, sc)
		defer cancelSDBCRollout()
		sc, err = controllerhelpers.WaitForScyllaDBClusterState(waitSDBCRolloutCtx, f.ScyllaAdminClient().ScyllaV1alpha1().ScyllaDBClusters(sc.Namespace), sc.Name, controllerhelpers.WaitForStateOptions{}, utils.IsScyllaDBClusterRolledOut)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		hostsByDC := make(map[string][]string, len(sc.Spec.Datacenters))
		...
		framework.By("Registering ScyllaDBCluster with the global ScyllaDB Manager instance")
		smClusterRegistration := &scyllav1alpha1.ScyllaDBManagerClusterRegistration{
		...
		smClusterRegistration, err = nsClient.ScyllaClient().ScyllaV1alpha1().ScyllaDBManagerClusterRegistrations(ns.Name).Create(ctx, smClusterRegistration, metav1.CreateOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		managerTask, err := managerClient.GetTask(ctx, managerClusterID, managerclient.BackupTask, managerTaskID)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		managerTask, err = managerClient.GetTask(updatePropagationCtx, managerClusterID, managerclient.BackupTask, managerTaskID)
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(managerTask.Properties.(map[string]interface{})["retention"].(json.Number).Int64()).To(o.Equal(*smt.Spec.Backup.Retention))
		...
		err = apimachineryutilwait.PollUntilContextTimeout(ctx, 5*time.Second, 3*time.Minute, true, func(context.Context) (done bool, err error) {
			backupProgress, err = managerClient.BackupProgress(ctx, managerClusterID, managerTask.ID, "latest")
			if err != nil {
				return false, err
			}
		...
		tasks, err := managerClient.ListTasks(ctx, managerClusterID, managerclient.BackupTask, false, "", "")
		...
		targetSC.Spec.ScyllaDB.Image = sc.Spec.ScyllaDB.Image
		...
		framework.By("Creating a target ScyllaDBCluster")
		...
		framework.By("Registering the target ScyllaDBCluster with the global ScyllaDB Manager instance")
		targetSMCR := &scyllav1alpha1.ScyllaDBManagerClusterRegistration{
			ObjectMeta: metav1.ObjectMeta{
				GenerateName: "smcr-",
				Namespace:    ns.Name,
			},
		...
		targetSMCR, err = nsClient.ScyllaClient().ScyllaV1alpha1().ScyllaDBManagerClusterRegistrations(ns.Name).Create(ctx, targetSMCR, metav1.CreateOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		for _, dcSpec := range targetSC.Spec.Datacenters {
			...
			// TODO: restart remote ScyllaDBDatacenter to ensure that the restored schema is applied and run tables restore.
		}
	})
})
