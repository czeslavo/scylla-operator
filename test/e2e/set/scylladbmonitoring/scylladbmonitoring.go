...
type scyllaDBMonitoringEntry struct {
	...
	PreparePrometheusFn          func(context.Context, *framework.Framework, *scyllav1alpha1.ScyllaDBMonitoring)
...
func describeEntry(e *scyllaDBMonitoringEntry) string {
	return fmt.Sprintf("with %s", e.WithDescription)
}

var _ = g.Describe("ScyllaDBMonitoring", func() {
	f := framework.NewFramework("scylladbmonitoring")

	expectedPlatformFolderDashboardSearchResponse, expectedPlatformHomeDashboardUID := getExpectedPlatformDashboards()
	...
	g.DescribeTable("should setup monitoring stack TESTCASE_DISABLED_ON_OPENSHIFT", func(ctx g.SpecContext, e *scyllaDBMonitoringEntry) {
		...
		sc := createSingleNodeScyllaCluster(ctx, f)
		...
		verifyScyllaClusterRollout(ctx, f, sc)

		if e.PreparePrometheusFn != nil {
			framework.By("Preparing Prometheus")
			e.PreparePrometheusFn(ctx, f, sm)
		}

		framework.By("Waiting for the ScyllaDBMonitoring to roll out (RV=%s)", sm.ResourceVersion)
		verifyScyllaDBMonitoringRollout(ctx, f, sm)
		...
		e.verifyManagedGrafana(ctx, f, sm)

	},
		g.FEntry(describeEntry, &scyllaDBMonitoringEntry{
			WithDescription: "SAAS type",
			...
			PreparePrometheusFn:      nil, // Using managed Prometheus.
			VerifyPrometheusFn:       verifyManagedPrometheus,
			ExpectedHomeDashboardUID: "cql-overview",
			ExpectedDashboards: []gapi.FolderDashboardSearchResponse{
				{
					Title:       "CQL Overview",
					Type:        "dash-db",
...
func createSingleNodeScyllaCluster(ctx context.Context, f *framework.Framework) *scyllav1.ScyllaCluster {
	g.GinkgoHelper()
...
func verifyScyllaClusterRollout(ctx context.Context, f *framework.Framework, sc *scyllav1.ScyllaCluster) {
	g.GinkgoHelper()
...
func verifyScyllaDBMonitoringRollout(ctx context.Context, f *framework.Framework, sm *scyllav1alpha1.ScyllaDBMonitoring) {
	g.GinkgoHelper()
...
func (e scyllaDBMonitoringEntry) verifyManagedGrafana(ctx context.Context, f *framework.Framework, sm *scyllav1alpha1.ScyllaDBMonitoring) {
	g.GinkgoHelper()
	...
	verifyGrafanaDashboards(ctx, f, verifyGrafanaDashboardsOptions{
		...
		ExpectedDashboards:   e.ExpectedDashboards,
	})
}
