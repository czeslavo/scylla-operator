...
func makeMirroredRemoteSecrets(sc *scyllav1alpha1.ScyllaDBCluster, dc *scyllav1alpha1.ScyllaDBClusterDatacenter, remoteNamespace *corev1.Namespace, remoteController metav1.Object, localSecretLister corev1listers.SecretLister, managingClusterDomain string) ([]metav1.Condition, []*corev1.Secret, error) {
	...
	agentAuthTokenSecretName, err := naming.ScyllaDBManagerAgentAuthTokenSecretNameForScyllaDBCluster(sc)
	if err != nil {
		return progressingConditions, requiredRemoteSecrets, fmt.Errorf("can't get agent auth token secret name for ScyllaDBCluster %q: %w", naming.ObjRef(sc), err)
	}

	secretsToMirror := []string{
		agentAuthTokenSecretName,
	}
...
func makeLocalSecrets(sc *scyllav1alpha1.ScyllaDBCluster, scyllaDBManagerAgentAuthTokenConfig []byte) ([]*corev1.Secret, error) {
	var localSecrets []*corev1.Secret

	scyllaDBManagerAgentAuthTokenSecret, err := makeLocalScyllaDBManagerAgentAuthTokenSecret(sc, scyllaDBManagerAgentAuthTokenConfig)
	if err != nil {
...
func makeLocalScyllaDBManagerAgentAuthTokenSecret(sc *scyllav1alpha1.ScyllaDBCluster, authTokenConfig []byte) (*corev1.Secret, error) {
	agentAuthTokenSecretName, err := naming.ScyllaDBManagerAgentAuthTokenSecretNameForScyllaDBCluster(sc)
	if err != nil {
