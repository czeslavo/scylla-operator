...
func TestValidateScyllaDBDatacenter(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedErrorList: field.ErrorList{
				&field.Error{Type: field.ErrorTypeInvalid, Field: "spec.rackTemplate.topologyLabelSelector", BadValue: "-123", Detail: `name part must consist of alphanumeric characters, '-', '_' or '.', and must start and end with an alphanumeric character (e.g. 'MyName',  or 'my.name',  or '123-abc', regex used for validation is '([A-Za-z0-9][-A-Za-z0-9_.]*)?[A-Za-z0-9]')`, Origin: "format=k8s-label-key"},
		...
		},
		{
			...
			expectedErrorList: field.ErrorList{
				&field.Error{Type: field.ErrorTypeInvalid, Field: "spec.rackTemplate.scyllaDB.storage.metadata.annotations", BadValue: "-123", Detail: `name part must consist of alphanumeric characters, '-', '_' or '.', and must start and end with an alphanumeric character (e.g. 'MyName',  or 'my.name',  or '123-abc', regex used for validation is '([A-Za-z0-9][-A-Za-z0-9_.]*)?[A-Za-z0-9]')`, Origin: "format=k8s-label-key"},
			},
...
func TestValidateScyllaArgsIPFamily(t *testing.T) {
	...
	tt := []struct {
		...
		},
		{
			...
			expectedErrorString: `test: Invalid value: "--rpc-address=2001:db8::1": --rpc-address '2001:db8::1' IP family (IPv6) must match spec.ipFamilies[0] (IPv4)`,
		},
		{
			...
			expectedErrorString: `test: Invalid value: "--listen-address=2001:db8::2": --listen-address '2001:db8::2' IP family (IPv6) must match spec.ipFamilies[0] (IPv4)`,
		},
		{
			...
			expectedErrorString: `test: Invalid value: "--rpc-address=10.0.0.1": --rpc-address '10.0.0.1' IP family (IPv4) must match spec.ipFamilies[0] (IPv6)`,
		},
		{
			...
			expectedErrorString: `test: Invalid value: "--listen-address=10.0.0.2": --listen-address '10.0.0.2' IP family (IPv4) must match spec.ipFamilies[0] (IPv6)`,
		},
		{
			...
			expectedErrorString: `[test: Invalid value: "--rpc-address=2001:db8::1 --listen-address=2001:db8::2": --rpc-address '2001:db8::1' IP family (IPv6) must match spec.ipFamilies[0] (IPv4), test: Invalid value: "--rpc-address=2001:db8::1 --listen-address=2001:db8::2": --listen-address '2001:db8::2' IP family (IPv6) must match spec.ipFamilies[0] (IPv4)]`,
		},
		{
		...
		},
		{
			...
			expectedErrorString: `test: Invalid value: "--rpc-address=10.0.0.1 --listen-address=2001:db8::2 --some-other-flag=value": --listen-address '2001:db8::2' IP family (IPv6) must match spec.ipFamilies[0] (IPv4)`,
		},
	}
